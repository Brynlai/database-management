...
-- If the index already exists, you may see ORA-01408 (safe to ignore).

PROMPT -- Confirm index exists
...
-- Option B (via view, if you created V_BUS_SCHEDULE_DETAILS)
-- SELECT driver_id,
--        driver_name,
--        license_no,
--        schedule_id,
...
-- Bind variable for plate number (change as needed)
...
-- View-based variant (if V_BUS_SCHEDULE_DETAILS exists):
-- SELECT schedule_id,
--        plate_number,
--        TO_CHAR(departure_time,'DD-MON-YYYY HH24:MI') AS dep_time,
--        TO_CHAR(arrival_time,  'DD-MON-YYYY HH24:MI') AS arr_time,
