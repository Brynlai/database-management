...
ticket_id_counter = 1
# FIX: These lists will now store rich dictionaries to enforce date logic.
schedules_data = []
booked_tickets_data = []
...
# --- Generator Functions ---
# NOTE: Only functions with logical changes are shown in full detail. Others are condensed.
...
def generate_schedules(f):
    ...
    for i in schedule_ids:
        ...
        if arr_time_obj > END_DATE_GLOBAL: continue # Skip schedules that end too late
        
        # FIX: Store schedule ID and its departure time for later use
...
def generate_booking_flow(f):
    ...
    for i in range(NUM_BOOKINGS):
        ...
        # FIX: Core logic change. Pick a schedule first to get its departure time.
        selected_schedule = random.choice(schedules_data)
        departure_time = selected_schedule['departure']
        schedule_id = selected_schedule['id']

        # FIX: Generate a booking date that is guaranteed to be BEFORE the departure.
        # Booking window is 1 to 90 days before departure.
        ...
        # Ensure the booking date doesn't go before our global start date
        ...
        for _ in range(num_tickets_in_booking):
            ...
            # FIX: Track ticket with its booking and departure dates for logical refunds/extensions
...
def generate_refunds(f):
    ...
    for i in range(num_to_refund):
        ...
        # FIX: Generate a refund date that is logically between booking and departure
    ...
    # Remove refunded tickets from the available pool for extensions
...
def generate_extensions(f):
    ...
    for i in range(num_to_extend):
        ...
        # FIX: Generate an extension date that is logically between booking and departure
        time_diff = ticket_info['departure'] - ticket_info['booking_date']
...
def generate_available_tickets(f, count):
    ...
    for _ in range(count):
        seat = f"{random.randint(1, 15)}{random.choice('ABCD')}"
        status = 'Available'
        schedule_id = random.choice(schedule_ids) # These don't need logical booking dates
        promo_id = 'NULL'
...
# --- Unchanged Generator Functions (Condensed for Brevity) ---
