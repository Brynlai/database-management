    ...
    NVL(
        SUM(
            CASE
                ...
                WHEN 'Fixed Amount' THEN p.discount_value
                ELSE 0
            END
        ),
        0
...
WHERE
    t.status = 'Booked'
    AND p.campaign_id = cp_campaign_id;
...
SELECT
    ...
    NVL(SUM(total_discounts), 0),
    NVL(SUM(net_revenue), 0),
    NVL(SUM(adj_profit), 0) INTO v_grand_tickets,
    v_grand_gross_rev,
    v_grand_discounts,
    v_grand_net_rev,
    v_grand_adj_profit
FROM
    (
        SELECT
            COUNT(t.ticket_id) AS tickets_sold,
            SUM(s.base_price) AS gross_revenue,
            SUM(
                CASE
                    p.discount_type
                    WHEN 'Percentage' THEN s.base_price * (p.discount_value / 100)
...
DBMS_OUTPUT.PUT_LINE(
    'Campaign Analytics Dashboard for Year: ' || p_year
);

DBMS_OUTPUT.PUT_LINE(RPAD('=', 128, '='));

DBMS_OUTPUT.PUT_LINE(
    RPAD('Campaign Name', 42) || LPAD('Promos', 8) || LPAD('Tickets', 10) || LPAD('Discounts (RM)', 18) || LPAD('Net Rev (RM)', 18) || LPAD('Adj. Profit (RM)', 18) || LPAD('Avg/Tkt (RM)', 14)
);

DBMS_OUTPUT.PUT_LINE(RPAD('-', 128, '-'));

FOR camp_rec IN campaign_cursor LOOP v_report_generated := TRUE;
...
DBMS_OUTPUT.PUT_LINE(
    RPAD(SUBSTR(camp_rec.campaign_name, 1, 40), 42) || LPAD(TO_CHAR(v_stats_rec.promo_count, 'FM990'), 8) || LPAD(TO_CHAR(v_stats_rec.tickets_sold, 'FM9,990'), 10) || LPAD(
...
IF v_report_generated THEN DBMS_OUTPUT.PUT_LINE(RPAD('-', 128, '-'));

DBMS_OUTPUT.PUT_LINE(
    RPAD('GRAND TOTALS:', 42) || LPAD(TO_CHAR(v_grand_promos, 'FM990'), 8) || LPAD(TO_CHAR(v_grand_tickets, 'FM9,990'), 10) || LPAD(
        TO_CHAR(v_grand_discounts, 'FM999,999,990.00'),
        18
    ) || LPAD(TO_CHAR(v_grand_net_rev, 'FM999,999,990.00'), 18) || LPAD(
        TO_CHAR(v_grand_adj_profit, 'SFM999,999,990.00'),
        18
    ) || LPAD(
        TO_CHAR(
            CASE
                WHEN v_grand_tickets > 0 THEN v_grand_net_rev / v_grand_tickets
                ELSE 0
            END,
            'FM990.00'
        ),
...
ELSE DBMS_OUTPUT.PUT_LINE(
    'No campaign data found for the year ' || p_year || '.'
);
...
WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE(
    'An error occurred while generating the report: ' || SQLERRM
);
...
CREATE
OR REPLACE PROCEDURE rpt_bus_maintenance_history (p_bus_id IN Bus.bus_id % TYPE) AS CURSOR bus_cursor IS
SELECT
    b.plate_number,
...
BEGIN FOR bus_rec IN bus_cursor LOOP v_bus_found := TRUE;
...
DBMS_OUTPUT.PUT_LINE(
    RPAD('Service Date', 14) || RPAD('Service Name', 28) || LPAD('Std Cost', 15) || LPAD('Actual Cost', 15) || LPAD('Variance', 15) || LPAD('Variance %', 12)
);

DBMS_OUTPUT.PUT_LINE(RPAD('-', 110, '-'));

FOR service_rec IN service_history_cursor(p_bus_id) LOOP v_variance := service_rec.actual_cost - service_rec.standard_cost;

DBMS_OUTPUT.PUT_LINE(
    RPAD(
        TO_CHAR(service_rec.service_date, 'DD-MON-YYYY'),
        14
    ) || RPAD(SUBSTR(service_rec.service_name, 1, 26), 28) || LPAD(
        TO_CHAR(service_rec.standard_cost, 'FM99,990.00'),
        15
    ) || LPAD(
...
DBMS_OUTPUT.PUT_LINE(
    'Overall Financial Summary for Bus ' || bus_rec.plate_number || ':'
);

DBMS_OUTPUT.PUT_LINE(
    RPAD('Total Maintenance Events:', 35) || LPAD(v_event_count, 20)
);

DBMS_OUTPUT.PUT_LINE(
    RPAD('Total Standard Cost:', 35) || 'RM ' || LPAD(TO_CHAR(v_total_standard, 'FM999,990.00'), 17)
);

DBMS_OUTPUT.PUT_LINE(
    RPAD('Total Actual Cost:', 35) || 'RM ' || LPAD(TO_CHAR(v_total_actual, 'FM999,990.00'), 17)
);

DBMS_OUTPUT.PUT_LINE(
    RPAD('Total Lifetime Variance:', 35) || 'RM ' || LPAD(
        TO_CHAR(
            v_total_actual - v_total_standard,
            'SFM999,990.00'
        ),
        17
    )
...
IF NOT v_bus_found THEN DBMS_OUTPUT.PUT_LINE(
    'Error: No bus found with ID ' || p_bus_id || '.'
);
...
--=============================================================================
PROMPT == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == = PROMPT DEMONSTRATING REPORTS PROMPT == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == =
SET
    LINESIZE 180;
...
PROMPT [SUCCESS CASE 1.1] Generating detailed analytics report for 2025.BEGIN rpt_campaign_analytics(p_year = > 2025);
...
PROMPT [SUCCESS CASE 2.1] Generating maintenance history for Bus ID 31 (a bus with service records).BEGIN rpt_bus_maintenance_history(p_bus_id = > 31);
