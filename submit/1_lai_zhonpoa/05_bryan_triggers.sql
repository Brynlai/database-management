...
CREATE
OR REPLACE TRIGGER trg_check_staff_assignment BEFORE
INSERT
    OR
UPDATE
...
WHEN NO_DATA_FOUND THEN RAISE_APPLICATION_ERROR(
    -20026,
    'Assignment Failed: Staff member with ID ' || :NEW.staff_id || ' does not exist.'
);

END;

-- Step 2: Validate the staff member's status.
IF v_staff_details.status != 'Active' THEN RAISE_APPLICATION_ERROR(
    -20025,
...
IF v_staff_details.role != :NEW.role THEN RAISE_APPLICATION_ERROR(
    -20027,
...
CREATE
OR REPLACE TRIGGER trg_prevent_company_deletion BEFORE DELETE ON Company FOR EACH ROW DECLARE v_bus_count NUMBER;

v_bus_examples VARCHAR2(200);

BEGIN -- Step 1: Check if this company owns any buses.
SELECT
    COUNT(*) INTO v_bus_count
FROM
    Bus
WHERE
...
RAISE_APPLICATION_ERROR(
    -20020,
    'Cannot delete company ''' || :OLD.name || '''. It owns ' || v_bus_count || ' bus(es). Please reassign them first. (Examples: ' || v_bus_examples || '...)'
);
...
--=============================================================================
PROMPT == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == = PROMPT DEMONSTRATING TRIGGERS PROMPT == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == = --
-- DEMO 1: trg_check_staff_assignment
--
PROMPT --- Testing Trigger 1: Invalid Staff Assignment ---
PROMPT [SETUP] Creating temporary staff
and service for a clean test...DECLARE v_service_id Service.service_id % TYPE;

v_bus_id Bus.bus_id % TYPE;

BEGIN -- Clean up from previous runs
DELETE FROM
    StaffAllocation
WHERE
    staff_id = 99903;
...
VALUES
    (99998, SYSDATE, 100, v_service_id, v_bus_id);
...
-- Role matches Staff table
DBMS_OUTPUT.PUT_LINE(
    'Success: Active staff member assigned to a matching role.'
);
...
INSERT INTO
    StaffAllocation(service_transaction_id, staff_id, role)
VALUES
    (99998, 99903, 'Technician');

-- Role MISMATCH
END;

/ PROMPT [FAILURE CASE 1.3] Assigning an inactive staff member.-- First, make the staff member inactive
UPDATE
    Staff
SET
    status = 'On Leave'
...
-- Status is now 'On Leave'
END;

/ PROMPT [CLEANUP] Removing temporary staff
and service data...BEGIN
DELETE FROM
    StaffAllocation
WHERE
    staff_id = 99903;
...
PROMPT [SETUP] Creating a temporary company
and assigning buses to it...BEGIN -- Clean up from previous runs
DELETE FROM
    Bus
WHERE
    company_id = 999;
...
VALUES
    (999, 'Temp Test Transport');

INSERT INTO
    Bus(bus_id, plate_number, capacity, company_id)
VALUES
    (bus_seq.NEXTVAL, 'TEST-001', 50, 999);

INSERT INTO
    Bus(bus_id, plate_number, capacity, company_id)
VALUES
    (bus_seq.NEXTVAL, 'TEST-002', 50, 999);

INSERT INTO
    Bus(bus_id, plate_number, capacity, company_id)
VALUES
    (bus_seq.NEXTVAL, 'TEST-003', 50, 999);
...
DBMS_OUTPUT.PUT_LINE(
    'Success: Company with ID 999 deleted after its buses were removed.'
);
