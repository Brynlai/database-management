...
CREATE
OR REPLACE PROCEDURE Add_New_Staff (
...
IF p_role NOT IN (
    ...
    -20003,
    'Validation Error: Invalid role specified. Must be one of: Counter Staff, Cleaner, Manager, Technician.'
);
...
DBMS_OUTPUT.PUT_LINE(
    'Success: New staff member ''' || p_name || ''' added with ID ' || o_new_staff_id || '.'
);
...
RAISE_APPLICATION_ERROR(
    -20001,
    'Error: A staff member with the email ''' || p_email || ''' already exists.'
);
...
CREATE
OR REPLACE PROCEDURE Reassign_Driver_For_Schedule (
...
IF v_schedule_count = 0 THEN RAISE_APPLICATION_ERROR(
    -20010,
    'Reassignment Failed: Schedule with ID ' || p_schedule_id || ' does not exist.'
);
...
IF v_new_driver_count = 0 THEN RAISE_APPLICATION_ERROR(
    -20011,
    'Reassignment Failed: New driver with ID ' || p_new_driver_id || ' does not exist.'
);
...
IF v_old_assignment_count = 0 THEN RAISE_APPLICATION_ERROR(
    -20013,
    'Reassignment Failed: Driver ' || p_old_driver_id || ' is not currently assigned to schedule ' || p_schedule_id || '.'
);
...
DBMS_OUTPUT.PUT_LINE(
    'Success: Reassigned schedule ' || p_schedule_id || ' from driver ' || p_old_driver_id || ' to driver ' || p_new_driver_id || '.'
);
...
RAISE_APPLICATION_ERROR(
    -20012,
    'Reassignment Failed: New driver ' || p_new_driver_id || ' is already assigned to schedule ' || p_schedule_id || '.'
);
...
--=============================================================================
PROMPT == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == = PROMPT DEMONSTRATING STORED PROCEDURES PROMPT == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == = --
-- DEMO 1: Add_New_Staff Procedure
--
PROMPT --- Testing Procedure 1: Add_New_Staff ---
PROMPT [SETUP] Cleaning up any previous test data for 'Add_New_Staff' BEGIN
DELETE FROM
    Staff
WHERE
    email = 'maria.garcia.demo@busstation.com';
...
/ PROMPT [SUCCESS CASE 1.1] Adding a new staff member with valid data.DECLARE v_new_id NUMBER;
...
PROMPT [FAILURE CASE 1.2] Attempting to
add
    a staff member with a duplicate email.DECLARE v_new_id NUMBER;

BEGIN Add_New_Staff(
    'John Doe',
    'Manager',
    'maria.garcia.demo@busstation.com',
    '555-5678',
    'Active',
    v_new_id
);
...
PROMPT [SETUP] Creating a fully isolated test environment...DECLARE v_bus_id Bus.bus_id % TYPE;

BEGIN -- Clean up any leftovers from a previous failed run to ensure idempotency
DELETE FROM
    DriverList
WHERE
    schedule_id = 99999;
...
VALUES
    (99901, 'Old Driver Bob', 'TEST-OLD-99901');

INSERT INTO
    Driver (driver_id, name, license_no)
VALUES
    (99902, 'New Driver Alice', 'TEST-NEW-99902');
...
INSERT INTO
    DriverList (schedule_id, driver_id, assignment_notes)
VALUES
    (99999, 99901, 'Initial assignment for test');
...
WHERE
    dl.schedule_id = 99999;

PROMPT [SUCCESS CASE 2.1] Reassigning a driver on the temporary schedule.BEGIN Reassign_Driver_For_Schedule(
    p_schedule_id = > 99999,
...
PROMPT [FAILURE CASE 2.2] Attempting to reassign a driver who is no longer assigned.BEGIN Reassign_Driver_For_Schedule(
    p_schedule_id = > 99999,
    p_old_driver_id = > 99901,
    -- Bob is no longer assigned, this should fail.
    p_new_driver_id = > 10,
    -- A random valid driver
    p_notes = > 'This should fail'
);

END;

/ PROMPT [CLEANUP] Removing all temporary test data...BEGIN
DELETE FROM
    DriverList
WHERE
    schedule_id = 99999;
